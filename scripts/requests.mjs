...
export function matchesRequestedCheck(result, row, payload) {
  ...
  if (
    check?.version !== 1 ||
    check.actorUuid !== row.uuid ||
    check.otf !== expected ||
    check.mode !== payload.mode
  )
    return false;
  const rolls = collection(result.rolls);
  if (
    rolls.length !== 1 ||
    !/^3d6(?:\s*\[[^\]]*\])?$/i.test(String(rolls[0].formula || '').trim()) ||
    !Number.isInteger(rolls[0].total) ||
    rolls[0].total < 3 ||
    rolls[0].total > 18
  )
    return false;
  const recipients = collection(result.whisper).map((u) => (typeof u === 'string' ? u : u.id));
  if (payload.mode === 'blindroll')
    return (
      result.blind === true &&
      recipients.length > 0 &&
...
export function wasRolled(message, index) {
  ...
  const payload = source?.getFlag(ID, 'requestData'),
    row = payload?.entries?.[ref.index];
  return (
    completed.has(ref.key) ||
    !!source?.getFlag(ID, 'responses')?.[ref.index] ||
    collection(game.messages).some(
      (m) =>
        m.author?.id === game.user.id &&
        m.isRoll &&
        m.getFlag(ID, 'roll') &&
        m.getFlag(ID, 'request')?.key === ref.key &&
        row &&
        matchesRequestedCheck(m, row, payload),
    )
  );
}
...
async function postRequest(payload, { audience, instruction = '', reminder = false }) {
  ...
  const data = {
    user: game.user.id,
    content: html,
    flags: { [ID]: { requestData: { ...payload, audience, instruction } } },
  };
  if (audience !== 'public')
    data.whisper = [
      ...new Set([...payload.users, ...ChatMessage.getWhisperRecipients('GM').map((u) => u.id)]),
    ];
...
export async function createRequests(
  rows,
  shortcut,
  { shared, reason, mode, audience, users, instruction },
) {
  if (!game.user.isGM) throw new Error('Only a GM can request rolls.');
  const entries = rows.map((r) => ({
    uuid: r.actor.uuid,
    name: r.entry.name || r.actor.name,
    entry: clone(r.entry),
    personal: integer(r.entry.adjustment || 0),
    managed: clone(modifiersFor(r.actor, shortcut)),
  }));
  const payload = {
    version: 1,
    entries,
    shortcut: clone(shortcut),
    shared: integer(shared),
    reason: modifierLabel(reason),
    mode: mode === 'publicroll' ? 'publicroll' : 'blindroll',
...
export async function requestStatus(message) {
  ...
  return Promise.all(
    payload.entries.map(async (entry, index) => {
      ...
      let actor;
      try {
        actor = await fromUuid(entry.uuid);
      } catch {}
      if (!actor)
        return { ...row, status: 'Unavailable', detail: 'Character or token no longer exists' };
      const owners = collection(game.users).filter(
        (u) => !u.isGM && payload.users.includes(u.id) && actor.testUserPermission(u, 'OWNER'),
      );
      if (!owners.length)
        return { ...row, status: 'Unavailable', detail: 'No addressed player owns this character' };
      try {
        const check = await resolveRoll(actor, payload.shortcut, entry.entry);
        if (check.missing || check.exempt)
          return {
            ...row,
            status: 'Unavailable',
            detail: check.exempt ? 'Exempt from this check' : 'Not found',
          };
      } catch {
        return { ...row, status: 'Unavailable', detail: 'Check cannot be resolved' };
      }
      return {
        ...row,
        status: 'Awaiting roll',
        detail: owners.map((u) => `${u.name}${u.active ? '' : ' (offline)'}`).join(', '),
        users: owners.map((u) => u.id),
      };
    }),
  );
}
export async function remindOutstanding(message) {
  ...
  const entries = pending.map((r) => ({
    ...clone(original.entries[r.index]),
    sourceIndex: r.index,
  }));
  return postRequest(
    { ...clone(original), entries, users, source: message.id },
    {
...
export async function recordResponse(result) {
  ...
  if (
    ...
    ref.index < 0 ||
    ref.key !== `${ref.messageId}:${ref.index}`
  )
    return;
  const original = game.messages.get(ref.messageId),
    payload = original?.getFlag(ID, 'requestData');
  if (
    !original?.author?.isGM ||
    payload?.version !== 1 ||
    original.getFlag(ID, 'responses')?.[ref.index]
  )
    return;
  const row = payload.entries[ref.index],
    user = result.author;
...
export function wireRequest(message, html) {
  ...
  for (const button of root.querySelectorAll('[data-gcs-request]')) {
    const index = Number(button.dataset.gcsRequest),
      ref = requestReference(message, index),
      key = ref.key;
    if (wasRolled(message, index)) {
      button.disabled = true;
      button.textContent = 'Rolled';
      continue;
    }
    if (payload.source && !game.messages.get(payload.source)) {
      button.disabled = true;
      button.textContent = 'Request removed';
      continue;
    }
    if (!message.author?.isGM || !payload.users?.includes(game.user.id)) {
      button.disabled = true;
      continue;
    }
    fromUuid(payload.entries[index]?.uuid || '')
      .then((actor) => {
        button.disabled =
          !actor?.testUserPermission(game.user, 'OWNER') ||
          inFlight.has(key) ||
          wasRolled(message, index);
        if (!actor?.testUserPermission(game.user, 'OWNER'))
          button.title = 'Only this character’s owner can roll.';
      })
      .catch(() => {
        button.disabled = true;
      });
    ...
    button.addEventListener('click', async (event) => {
      ...
      try {
        ...
        if (payload.source && !game.messages.get(payload.source))
          throw new Error('The original request has been deleted.');
        const row = payload.entries[index];
        if (!row || payload.version !== 1)
          throw new Error('This roll request is no longer supported.');
        const actor = await fromUuid(row.uuid);
        if (!actor || !actor.testUserPermission(game.user, 'OWNER'))
          throw new Error('You do not own this character.');
        validateAction(
          GURPS.parselink(row.entry.overrides?.[payload.shortcut.id] || payload.shortcut.otf)
            .action,
        );
        const result = await rollOne({
          ...
          shared: integer(payload.shared),
          reason: modifierLabel(payload.reason),
          personal: integer(row.personal),
          mode: payload.mode === 'publicroll' ? 'publicroll' : 'blindroll',
          includeBucket: true,
          request: ref,
          managed: row.managed,
        });
        if (['Success', 'Failure'].includes(result.status)) {
          ...
          ui.notifications.info(
            `Roll made for ${row.name}${payload.mode === 'blindroll' ? '; the result is visible only to GMs' : ''}.`,
          );
        } else {
