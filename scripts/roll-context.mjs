...
export function initialiseRollAdapters() {
  ...
  if (!globalThis.libWrapper?.register) throw new Error('Install and enable libWrapper to use GM Control Sheet rolls.');
  const required = [GURPS.ModifierBucket?.applyMods, GURPS.applyModifierDesc,
    GURPS.setLastTargetedRoll, ChatMessage.getSpeaker, ChatMessage.create];
  if (required.some(f=>typeof f !== 'function')) throw new Error('This GGA/Foundry version does not expose the required roll integration.');
  const installed = [];
  const wrap = (target, fn) => {libWrapper.register(ID, target, fn, 'MIXED'); installed.push(target);};
  try {
    ...
    wrap('GURPS.applyModifierDesc', function(wrapped, actor, desc) {
      ...
      return (async()=>{
        if (desc === context.capDescription) {context.capUsed = true; return context.cap;}
        const cap = await wrapped(context.actor, desc);
        if (cap && (!Number.isFinite(cap) || cap <= 0)) throw new Error('GGA returned an unsupported target limit.');
    ...
    wrap('GURPS.setLastTargetedRoll', function(wrapped, data, actorId, tokenId, broadcast) {
      ...
      return wrapped(data, context.speaker.actor, context.speaker.token, context.mode === 'blindroll' ? false : broadcast);
    });
...
export function validateResult(context, data = context.result) {
  if (!context.bucketUsed) throw new Error('GGA bypassed the isolated modifier list; the check was not posted.');
  if (!data || !Number.isFinite(data.finaltarget) || !Number.isFinite(data.rtotal)) throw new Error('GGA did not return a valid targeted roll; the check was not posted.');
  if (context.fright && (!context.capUsed || data.finaltarget > 13 || (data.rtotal >= 14 && !data.failure))) {
...
export function createRollContext(actor, entry, modifiers, fright, mode, request, otf) {
  ...
  const context = {id, actor, speaker, modifiers:clone(modifiers), fright, cap:13, capUsed:false,
    capDescription:'GM Control Fright Check limit', mode, request, otf, messages:[], bucketUsed:false};
  ...
  context.facade = Object.freeze({id:actor.id, uuid:actor.uuid, name:actor.name, isSelf:true, getOwners(){
...
export async function publishRollContext(context) {
  ...
  if (context.messages.filter(m=>m.data.rolls?.length).length !== 1) throw new Error('GGA returned an unexpected number of roll messages; the check was not posted.');
  for (const {data, options} of context.messages) {
    const clean = {...data, speaker:{...context.speaker}, flags:{...data.flags}};
    if (data.rolls?.length) clean.flags[ID] = {...data.flags?.[ID], roll:true, request:context.request,
    ...
    if (clean.blind && !clean.whisper.length) throw new Error('No GM recipients are available; the private check was not posted.');
