...
function fitHeights(wanted, minimum, total) {
  ...
  return room > 0
    ? heights.map((n, i) => n - (Math.max(0, excess) * (n - minimum[i])) / room)
    : heights;
}
...
export class SectionLayout {
  constructor(root, sizes, onChange, onEnd) {
    ...
    for (const panel of this.panels.slice(1))
      this.observer.observe(panel.querySelector('.gcs-action-title'));
  ...
  measureMinimum() {
    return [
      ...
      ...this.panels
        .slice(1)
        .map(
          (panel) => panel.querySelector('.gcs-action-title').getBoundingClientRect().height + 44,
        ),
    ];
  }
  heights() {
    return this.panels.map((panel) => panel.getBoundingClientRect().height);
  }
  layout() {
    if (this.drag || this.destroyed || !this.root.isConnected) return;
    this.minimum = this.measureMinimum();
    const dividerHeight = this.dividers.reduce(
      (sum, divider) => sum + divider.getBoundingClientRect().height,
      0,
    );
    ...
    if (!wanted) {
      const natural = this.panels.slice(1).map((panel) => {
        ...
        const contentHeight =
          last.getBoundingClientRect().bottom - body.getBoundingClientRect().top + body.scrollTop;
        return (
          panel.querySelector('.gcs-action-title').getBoundingClientRect().height +
          contentHeight +
      ...
      const results = Math.max(
        this.minimum[2],
        Math.min(natural[1], this.root.clientWidth <= 740 ? 130 : 190),
      );
  ...
  apply(heights) {
    heights.forEach((height, i) => {
      this.panels[i].style.flex = `0 0 ${height}px`;
    });
    this.dividers.forEach((divider, i) => {
      divider.setAttribute('aria-valuemin', Math.round(this.minimum[i]));
      divider.setAttribute(
        'aria-valuemax',
        Math.round(heights[i] + heights[i + 1] - this.minimum[i + 1]),
      );
      divider.setAttribute('aria-valuenow', Math.round(heights[i]));
      divider.setAttribute(
        'aria-valuetext',
        `${names[i]} ${Math.round(heights[i])} pixels; ${names[i + 1]} ${Math.round(heights[i + 1])} pixels`,
      );
    });
  }
  resize(index, heights, delta) {
    ...
    const movement = Math.max(
      this.minimum[index] - next[index],
      Math.min(delta, next[index + 1] - this.minimum[index + 1]),
    );
    next[index] += movement;
