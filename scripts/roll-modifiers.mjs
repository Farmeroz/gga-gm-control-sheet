...
export async function collectTaggedModifiers(actor, chatthing, optionalArgs, entry = {}) {
  ...
  if (typeof actor[helper] !== 'function')
    throw new Error('GGA does not expose the required tagged-modifier calculator.');
  const selected = new Set([
    ...tags(settings.allRolls),
    ...tags(optionalArgs.obj?.modifierTags),
    ...(optionalArgs.obj
      ? actor[helper](chatthing, settings, optionalArgs)
      : actor[helper](chatthing, settings)),
  ]);
  const itemRef =
    optionalArgs.obj?.name ||
    optionalArgs.obj?.originalName ||
    actor._getItemRefFromCheck?.(chatthing);
  const localise = (text) => {
    const key = text.match(/(GURPS\.\w+)/)?.[1];
    return key ? game.i18n.localize(key) + text.replace(key, '') : text;
  };
  const mods = [
    ...(actor.system?.conditions?.usermods || []),
    ...(actor.system?.conditions?.self?.modifiers || []).map(localise),
  ];
  const targets = list(game.user.targets);
  if (targets.length) {
    const native = await import('../../../systems/gurps/module/actor/effect-modifier-popout.js');
    if (
      typeof native.getRangedModifier !== 'function' ||
      typeof native.getSizeModifier !== 'function'
    )
      throw new Error('GGA does not expose the required target-modifier calculators.');
    const active = list(canvas.tokens?.placeables);
    const source = entry.tokenUuid
      ? active.find((t) => t.document?.uuid === entry.tokenUuid)
      : actor.token?.object || actor.getActiveTokens?.()[0];
    for (const target of targets) {
      mods.push(...(target.actor?.system?.conditions?.target?.modifiers || []).map(localise));
      if (source)
        mods.push(native.getRangedModifier(source, target), native.getSizeModifier(source, target));
    }
  }
  const inCombat =
    !!game.combat?.isActive &&
    list(game.combat.combatants).some((c) =>
  ...
  for (const text of mods.filter(Boolean)) {
    for (const tag of (text.match(/#(\S+)/g) || []).map((s) => s.slice(1).toLowerCase())) {
      ...
      if (
        ...
        !(itemRef && text.includes(itemRef)) &&
        !text.includes('@man:')
      )
        continue;
      if (
        inCombat
          ? settings.nonCombatOnlyTag && selected.has(settings.nonCombatOnlyTag)
          : settings.combatOnlyTag && selected.has(settings.combatOnlyTag)
      )
        continue;
      ...
      if (existing) {
        existing.modint += value;
        existing.mod = signed(existing.modint);
...
export function bucketSnapshot(bucket) {
  return bucket.modifierStack.modifierList
    .filter((m) => !m.tagged)
    .map((value) => ({ value, copy: clone(value), fingerprint: JSON.stringify(value) }));
}
export function consumeSnapshot(bucket, snapshot) {
  const current = bucket.modifierStack.modifierList;
  const removable = new Set(
    snapshot.filter((s) => JSON.stringify(s.value) === s.fingerprint).map((s) => s.value),
  );
  ...
  if (remaining.length !== current.length) {
    ...
    bucket.modifierStack.sum();
    bucket.refresh();
  }
  if (changed)
    ui.notifications.warn(
      'Your bucket changed during the check.  Changed modifiers were kept; review them before your next roll.',
    );
}
