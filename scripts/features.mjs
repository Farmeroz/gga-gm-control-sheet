...
export function conditionHighlights(actor) {
  ...
  for (const [stat, state] of [
    ...
    const low =
      ...
      max > 0 &&
      value < max / 3;
    if (low || flags[state] || statuses.has(state))
      alerts.push({
        stat,
        severity: value <= 0 ? 'critical' : 'warning',
        label: low ? `Low ${stat}` : stat === 'HP' ? 'Reeling' : 'Exhausted',
        detail: low
          ? `${stat} below one-third of maximum`
          : `${stat === 'HP' ? 'Reeling' : 'Exhausted'} marked in GGA`,
      });
  }
  if (statuses.has('stun') || statuses.has('stunned'))
    alerts.push({ label: 'Stunned', detail: 'Stun marked in GGA', severity: 'warning' });
  if (statuses.has('mentalstun'))
    alerts.push({ label: 'Mental stun', detail: 'Mental stun marked in GGA', severity: 'warning' });
  if (statuses.has('disabled'))
    alerts.push({ label: 'Disabled', detail: 'Disabled marked in GGA', severity: 'critical' });
  if (statuses.has('unconscious'))
    alerts.push({
      label: 'Unconscious',
      detail: 'Unconsciousness marked in GGA',
      severity: 'critical',
    });
  if (statuses.has('dead'))
    alerts.push({ label: 'Dead', detail: 'Death marked in GGA', severity: 'critical' });
  return alerts;
}
...
export function validatePreset(preset, shortcutIds) {
  const checks =
    preset.checks === 'all'
      ? 'all'
      : Array.isArray(preset.checks)
        ? [...new Set(preset.checks.map(safeId))]
        : [];
  if (checks !== 'all' && (!checks.length || checks.some((id) => !shortcutIds.has(id))))
    throw new Error('Choose existing checks for this preset.');
  return {
    id: safeId(preset.id),
    label: textValue(preset.label, 'Preset name', 60),
    name: modifierLabel(preset.name),
    value: integer(preset.value),
    checks,
  };
}
export function exportConfiguration(prefs) {
  const known = new Set(
    Object.values(prefs.shortcuts)
      .flat()
      .map((s) => s.id),
  );
  for (const preset of prefs.presets || []) {
    if (preset.checks !== 'all' && preset.checks.some((id) => !known.has(id)))
      throw new Error(
        `Preset “${preset.label}” refers to removed checks.  Edit it under Configure → Presets before exporting.`,
      );
  }
  return {
    format: ID,
...
export function parseConfiguration(text, parseAction) {
  ...
  if (data?.format !== ID || data.version !== 1 || !data.configuration)
    throw new Error('This is not a supported GM Control Sheet configuration.');
  const source = data.configuration;
  if (!Array.isArray(source.columns) || source.columns.some((c) => !Object.hasOwn(COLUMNS, c)))
    throw new Error('Unknown statistic in configuration.');
  const columns = [...new Set(source.columns)];
  const shortcuts = {},
    identities = new Map();
  for (const group of ['pc', 'npc']) {
    if (
      !Array.isArray(source.shortcuts?.[group]) ||
      !source.shortcuts[group].length ||
      source.shortcuts[group].length > 100
    )
      throw new Error('Keep between 1 and 100 checks in each roster.');
    shortcuts[group] = source.shortcuts[group].map((s) => {
      const id = safeId(s.id),
        label = textValue(s.label, 'Shortcut label', 60),
        otf = textValue(s.otf, 'OtF');
      validateAction(parseAction(otf));
      if (identities.has(id) && identities.get(id) !== normalise(label))
        throw new Error('A check identifier is used for different labels.');
      identities.set(id, normalise(label));
      return { id, label, otf };
    });
    if (
      new Set(shortcuts[group].map((s) => s.id)).size !== shortcuts[group].length ||
      new Set(shortcuts[group].map((s) => normalise(s.label))).size !== shortcuts[group].length
    )
      throw new Error('Check names and identifiers must be unique in each roster.');
  }
  if (!Array.isArray(source.presets) || source.presets.length > 100)
    throw new Error('Invalid preset list.');
  const presets = source.presets.map((p) => validatePreset(p, new Set(identities.keys())));
  if (
    new Set(presets.map((p) => normalise(p.label))).size !== presets.length ||
...
export function importConfiguration(prefs, incoming, randomID) {
  ...
  const ids = new Map(),
    used = new Set(existing.map((s) => s.id));
  ...
  result.presets = incoming.presets.map((p) => ({
    ...p,
    id: randomID(),
    checks: p.checks === 'all' ? 'all' : p.checks.map((id) => ids.get(id)),
  }));
  return result;
}
