...
export const DEFAULT_SHORTCUTS = [
  ['per', 'Per', 'Per'], ['vision', 'Vision', 'Vision'], ['hearing', 'Hearing', 'Hearing'],
  ['observation', 'Observation', 'S:"Observation"'], ['search', 'Search', 'S:"Search"'],
  ['will', 'Will', 'Will'], ['fright', 'Fright Check', 'Fright Check'],
  ['dx', 'DX', 'DX'], ['iq', 'IQ', 'IQ'], ['ht', 'HT', 'HT'],
].map(([id, label, otf]) => ({id, label, otf}));
export const COLUMNS = {
  HP: 'HP', FP: 'FP', ST: 'ST', DX: 'DX', IQ: 'IQ', HT: 'HT', PER: 'Per', WILL: 'Will',
  move: 'Move', dodge: 'Dodge', posture: 'Posture', maneuver: 'Manoeuvre', conditions: 'Conditions',
};
  ...
  if ((prefs.schemaVersion || 0) < 1) {
    for (const group of ['pc', 'npc']) {
      for (const addition of DEFAULT_SHORTCUTS.filter(s => ['dx', 'iq'].includes(s.id))) {
        const exists = prefs.shortcuts[group].some(s => s.id === addition.id ||
          s.otf.trim().replace(/^\[(.*)\]$/, '$1').toUpperCase() === addition.otf);
...
export function tableLayout(columns) {
  const weights = [36, 240, ...columns.map(c => ['HP','FP'].includes(c) ? 88 :
    ['posture','maneuver','conditions'].includes(c) ? 116 : 64), 116, 80, 80];
...
export const modifierName = s => String(s ?? '').replace(/\s*\(GM Control\)$/i, '').trim();
...
export function textValue(value, label, max = 160) {
  ...
  if (!text || text.length > max || /[\r\n\x00-\x1f]/.test(text)) throw new Error(`${label} is required (maximum ${max} characters).`);
  return text;
}
export function modifierLabel(value) {
  ...
  if (/[\[\]<>*@#]/.test(text)) throw new Error('Use plain text for the modifier description, without brackets, tags, or OtF directives.');
...
export function uniqueActors(rows) {
  const seen = new Set();
  return rows.filter(r => { const key = r.actor?.uuid; if (!key || seen.has(key)) return false; seen.add(key); return true; });
}
...
export function isFright(otf) { return /^!?\s*fright\s*check(?:\s|[+-]|$)/i.test(otf.trim()); }
export function isUnfazeable(actor, entry = {}) {
  if (entry.fright === 'exempt') return true;
  if (entry.fright === 'roll') return false;
  return walkList(actor?.system?.ads).some(a => /^unfazeable(?:\s*\(|$)/i.test(a.name?.trim() || ''));
}
export function modifiersFor(actor, shortcut) {
  return (actor.getFlag?.(ID, 'modifiers') || []).filter(m => m.checks === 'all' || m.checks?.includes(shortcut.id));
}
...
export function rollModifiers(managed, extra, bucket = []) {
  ...
  return [
    ...
    ...extra.filter(m => m.value !== 0).map(m => ({mod: signed(m.value), modint: m.value, desc: m.name})),
  ];
}
export function bucketPacket(mods) {
  return { modifierList: mods.map(m => ({mod: signed(m.value), modint: m.value, desc: modifierDesc(m.name), plus: m.value >= 0, tagged: false})),
    currentSum: mods.reduce((n,m) => n + m.value, 0), displaySum: signed(mods.reduce((n,m) => n + m.value, 0)) };
}
...
export function validateAction(action) {
  if (!action || !['attribute', 'skill-spell', 'controlroll'].includes(action.type)) throw new Error('Use a single attribute, skill, spell, or self-control OtF.');
  if (action.next || action.sourceId || action.costs || action.truetext || action.falsetext || action.followon) throw new Error('Chained rolls, actor prefixes, costs, and scripted follow-ups are not supported in shortcuts.');
  return action;
}
export function sanitiseObject(obj) {
  if (!obj) return undefined;
  return {name: obj.name, originalName: obj.originalName, modifierTags: obj.modifierTags || '',
    college: clone(obj.college), consumeAction: false};
}
