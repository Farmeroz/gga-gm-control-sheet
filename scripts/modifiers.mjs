...
export async function applyCharacterModifier(rows, data) {
  ...
  const mod = {
    ...
    name: modifierLabel(data.name),
    value: integer(data.value),
    checks: data.checks,
  };
  if (mod.checks !== 'all' && (!Array.isArray(mod.checks) || !mod.checks.length))
    throw new Error('Choose at least one check for this modifier.');
  const errors = [];
  let count = 0;
  for (const { actor } of uniqueActors(rows)) {
    try {
      await actor.setFlag(
        ID,
        'modifiers',
        upsertModifier(actor.getFlag(ID, 'modifiers') || [], mod),
      );
      count++;
    } catch (e) {
...
export async function removeCharacterModifiers(rows, names) {
  ...
  for (const { actor } of uniqueActors(rows)) {
    try {
      ...
      await actor.setFlag(ID, 'modifiers', remaining);
      count++;
    } catch (e) {
      errors.push(`${actor.name}: ${e.message}`);
    }
...
export async function sendBuckets(users, modifiers, add) {
  ...
  if (recipients.some((u) => !u.active))
    throw new Error(
      'One or more recipients went offline.  Review the recipient list and send again.',
    );
  ...
  if (remote.length)
    game.socket.emit('system.gurps', {
      type: 'updatebucket',
      users: remote.map((u) => u.id),
      bucket: clone(packet),
      add: !!add,
    });
  if (recipients.some((u) => u.id === game.user.id)) {
    if (add)
      for (const mod of packet.modifierList) GURPS.ModifierBucket.addModifier(mod.mod, mod.desc);
